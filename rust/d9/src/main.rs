...
fn main() {
    ...
    println!("[Part Two] Solution: {:?}", range_sum.iter().min().unwrap() + range_sum.iter().max().unwrap());
...
fn invalid_sum(input: Vec<i64>, preamble_length: usize) -> i64 {
    ...
    for tested_number in input[preamble_length..].iter() {
        let sum_completions: Vec<i64> = preamble.clone().iter().map(|n| tested_number - n).collect();
